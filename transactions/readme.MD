...
Ability to update multiple different records as part of a single ACID transaction which is guaranteed to either succeed or fail as a whole
...
* For the database cluster you deployed and shown in the Atlas console, click the __Connect button__, select __Connect with the Mongo Shell__, select __Short SRV Connection String__ and choose to __COPY__
...
```
mongo "mongodb+srv://testcluster-x123x.mongodb.net/test" --username main_user 
```

__3. Create a Database and Insert Some Initial Data__
* Using the Shell create the _game_ database and populate a _player_ collection, by inserting player documents into the collection, and then create a unique index on the player name, so no two players can have the same name in the game:
...
### TEST 3: Multiple Document Updates with Atomicity, Consistency wit a Rollback
...
* __RESULT 1__: Before the transaction commits, for a single update, other sessions should not see changes made a separate transaction, and only see them when the transaction is committed
* __RESULT 2__: Before the transaction commits, for multiple updates, other sessions should not see changes made a separate transaction, and only see them when the transaction is committed
